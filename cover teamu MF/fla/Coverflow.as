package {
	...
	public class Coverflow extends Sprite {
		...
		private function loadXML():void {
			...
			if(xml_source == null) {
				xml_source = 'xml/team.xml';
			}
			
			// loading the cover xml here
			coverflowXMLLoader = new URLLoader();
			coverflowXMLLoader.load(new URLRequest("xml/team.xml"));
		...
		private function coverflowXMLLoader_Complete(e:Event):void {
			...
			coverflowSpacing=55;
			coverflowImageWidth=220;
			coverflowImageHeight=220;
		...
		private function loadCover():void {
			...
			for (var i:int = 0; i < coverflowItemsTotal; i++) {
				var cover:Sprite=createCover(i,_data[i].image,_data[i].title);
				coverArray[i]=cover;
				cover.y=centerY;
				cover.z=0;
		...
		private function gotoCoverflowItem(n:int):void {
			if(n<0 || n>coverflowItemsTotal-1)
				n = _currentCover;
			_currentCover=n;
			reOrderCover(n);
			if (coverSlider) {
		...
		private function reOrderCover(currentCover:uint):void {
			for (var i:uint = 0, len:uint = coverArray.length; i < len; i++) {
				...
				if (i<currentCover) {
					...
					TweenLite.to(cover, transitionTime, {scaleX:0.3,scaleY:0.3,alpha:1,x:(centerX-20+(i - currentCover)*coverflowSpacing), z:(centerCoverflowZPosition), y:111});
					...
					//Center Coverflow
					//trace((cover.getChildAt(0).getChildAt(0)));
					TweenLite.to(cover, transitionTime, {scaleX:0.5,scaleY:0.5,alpha:1,x:centerX, z:centerCoverflowZPosition, y:90});
					
					trace(cover);
					
 
					ExternalInterface.call("change_image_action",cover.name);
					ExternalInterface.addCallback("changeCover",getTextFromJavaScript);
		...
		private function createCover(num:uint, url:String, title2:String):Sprite {
